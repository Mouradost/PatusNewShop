...
class DashBoard(QWidget):
    def __init__(self, **kwargs) -> None:
        super(DashBoard, self).__init__(
            objectName="w_reportDashBoard", **kwargs)
        ...
        self.de_dashboard_date_start.setDateTime(QDateTime.fromString(
            self.start_date, "yyyy-MM-dd"))
        self.de_dashboard_date_end.setDateTime(QDateTime.fromString(
            self.end_date, "yyyy-MM-dd"))
        # Connecting the dates
        self.de_dashboard_date_start.dateTimeChanged.connect(
            self.set_dates)
        self.de_dashboard_date_end.dateTimeChanged.connect(
            self.set_dates)
        self.set_dates()
    ...
    def set_dates(self) -> None:
        self.start_date = self.de_dashboard_date_start.dateTime().toString("yyyy-MM-dd")
        self.end_date = self.de_dashboard_date_end.dateTime().toString("yyyy-MM-dd")
    ...
    def show_worker_hours_ranking_(self) -> None:
        data = self.DB.getWorkerHourRankingByDateRange(
        ...
        show_dynamic(
            ...
            "Employee Attendance",
            ["Name", "Duration"],
            dict(sorted(ranking_dict.items(),
    ...
    def show_totals(self) -> None:
        ...
        # Showing the data
        self.l_dashboard_total_earning.setText(
            to_money(total_sells))
        self.l_dashboard_total_spending.setText(
            to_money(total_expenses))
        self.l_dashboard_total_gain.setText(
            to_money(total_gain))
        if total_gain > 0:
            self.l_dashboard_total_gain.setStyleSheet(
                "QLabel { color: #24ff50; }")
        else:
            self.l_dashboard_total_gain.setStyleSheet(
                "QLabel { color: #9e2800; }")
    ...
    def show_customer_ranking(self) -> None:
        ranking = self.DB.getCustomerRankingByDateRange(
            self.start_date,
            self.end_date)
        show_dynamic(
    ...
    def show_worker_ranking(self) -> None:
        ranking = self.DB.getWorkerRankingByDateRange(
            self.start_date,
            self.end_date)
        show_dynamic(
...
def show_dynamic(container: QFrame, title: str, fields: List[str],  ranking: List, field_w: str, wrapper: Callable) -> None:
    ...
    for i, results in enumerate(ranking):
        ...
        for field, result in zip(fields, results):
            label = QLabel()
            if field == field_w and wrapper is not None:
